...
test.beforeEach(async (t) => {
  ...
  t.context.dataVersion = Math.random() * 20;
  ...
  t.context.collectionConfigKey = (dataType, dataVersion) =>
    `${t.context.stackName}/collections/${dataType}__${dataVersion}.json`;
});
...
test('get() fetches a collection config from S3', async (t) => {
  await s3().putObject({
    ...
    Key: t.context.collectionConfigKey(t.context.dataType, t.context_dataVersion),
...
test('get() throws an exception if the bucket does not exist', async (t) => {
  ...
  try {
    await collectionConfigStore.get(t.context.dataType, t.context.dataVersion, t.context.dataVersion);
    t.fail('Expected an error to be thrown');
  }
  catch (err) {
...
test('delete() the collection conf ig from the cache', async (t) => {
  ...
  try {
    ...
    t.is(err.message, `A collection config for data type "${t.context.dataType} ${t.context.dataVersion}" was not found.`);
  }
});
